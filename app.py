...
MAPA_NORM = {normalize_key(k): v for k, v in VALORES_PARCEIROS.items()}
FUNCEF_NORM = normalize_key(PARCEIRO_FUNCEF)


def valor_ajustado(row):
    ...
    if parceiro_norm == FUNCEF_NORM:
        return valor_original, "Parceiro FUNCEF: valor da planilha"

    if parceiro_norm in MAPA_NORM:
        return float(MAPA_NORM[parceiro_norm]), "Parceiro: tabela fixa"
...
def preparar_base(df: pd.DataFrame) -> pd.DataFrame:
    d = df.copy()
    vals = d.apply(valor_ajustado, axis=1, result_type="expand")
...
def business_days_between(start_date, end_date):
    """Conta dias úteis de segunda a sexta entre duas datas, inclusive."""
    if start_date is None or end_date is None:
        return 0
    rng = pd.date_range(start=start_date, end=end_date, freq="D")
    return int(sum(d.weekday() < 5 for d in rng))
...
uploaded = st.sidebar.file_uploader("Anexe a planilha Excel do CRM", type=["xlsx", "xlsm", "xls"])

if not uploaded:
    st.info("Anexe a planilha para carregar o dashboard.")
    st.stop()
...
# Previsão corrigida:
# - O mês base é o "Mês de análise" selecionado na barra lateral.
# - Se o período selecionado já chega ao último dia do mês, a previsão é igual ao realizado.
# - Se o mês ainda está parcial, calcula média por dia útil decorrido e projeta para os dias úteis totais do mês.
mes_ini = mes_inicio
mes_fim_previsao = mes_fim
dias_uteis_mes = business_days_between(mes_ini, mes_fim_previsao)
data_corte_previsao = min(dt_fim, mes_fim_previsao)
dias_uteis_decorridos = business_days_between(mes_ini, data_corte_previsao)
...
if data_corte_previsao >= mes_fim_previsao:
    ...
    status_previsao = f"Mês parcial: projeção por média de dias úteis até {pd.to_datetime(data_corte_previsao).strftime('%d/%m/%Y')}."
